...
| `block_merge`                          | No       | `false`         | When `true`, fail the workflow (block the merge) when the Upwind backend decides the scan violates the PR-block policy. The threshold and severities are owned by the backend (currently: at least one introduced critical vulnerability). |
