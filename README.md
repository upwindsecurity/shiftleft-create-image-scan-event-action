...
| `block_merge`                          | No       | `false`         | When `true`, fail the workflow (block the merge) if the scan violates the Upwind PR-block policy (server-decided; currently at least one introduced vulnerability). |
