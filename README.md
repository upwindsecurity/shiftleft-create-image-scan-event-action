...
| `block_merge`                          | No       | `false`         | When `true`, fail the workflow (block the merge) if the scan result violates the Upwind blocking policy (default policy: at least one introduced critical vulnerability). |
